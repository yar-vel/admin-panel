...
const props = defineProps<{
  initialRows?: IResource[]
  initialMeta?: IResListMeta<IResource>
}>()
...
const selectedRows = ref<IResource['id'][]>([])
const {
  data: glData,
  error: glError,
  execute: glExecute,
  status: glStatus,
} = resourcesApi.getList({ reqPage, reqLimit, reqCount })
const {
  error: dError,
  execute: dExecute,
  status: dStatus,
} = resourcesApi.delete({ items: selectedRows })
...
<template>
  <div class="mb-3">
    <NuxtLink :to="rights.creating ? ROUTES.ui.newResource : undefined">
      <v-btn
        class="me-2"
        color="info"
